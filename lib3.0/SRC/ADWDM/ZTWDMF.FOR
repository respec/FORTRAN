...
C     + + + EXTERNALS + + +
      ...
      EXTERNAL    WMSSKB, WDNXPS, ZSTCMA, WMSDLM, WMSBCX
C
C     + + + SAVES + + +
...
C     set maximum output length
      LMAX= OLEN
C
      IF (INITFG.EQ.1 .AND. DSN.GT.0) THEN
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     1            LAPOS(30),LIMIN(30),LIMAX(30),LIDEF(30),LNMHDR,RETCOD
      ...
      CHARACTER*1 LFTYP(30),HDRBUF(78,10)
C
C     + + + ARGUMENT DEFINITIONS + + +
...
C
C     + + + PURPOSE + + +
C     Get help pointers for the current parm2 screen.
C     * * * This routine is specific to GenScn          * * *
C     * * * and resides here to keep AIDE common blocks * * *
C     * * * within the same code group.                 * * *
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER     GENPTR,FPTR(64)
C
C     + + + ARGUMENT DEFINITIONS + + +
C     GENPTR - pointer to general help
C     FPTR   - pointers to field specific help
C
C     + + + COMMON BLOCKS + + +
      INCLUDE 'zcntrl.inc'
C
