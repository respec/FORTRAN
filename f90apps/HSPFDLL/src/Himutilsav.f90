...
! last modified: 11-Mar-2002
      ...
      REAL          Values(Nval)
    !  CHARACTER*25  Binname
      Integer        lei
      CHARACTER(len=*) ::  Binname     
    !  CHARACTER*1   NameString(Nval*10)
    !  CHARACTER*8   Opname,Section
       CHARACTER(len=*) ::  NameString, Opname,   Section 
      ...
      DATA I0,I1,BINU,x/0,1,110,111/

      DLL_EXPORT Write_Binout

      ERRFLG = 0
!      write(*,*) 'Write_Binout:First ',First
      !read(*,*) i
      IF (First .eq. 0) THEN
        ...
        !do 10 i= 1, nval
          !k1= 10*(i-1)+1
          !k2= 10*(i-1)+NameLen(i)
          !WRITE (x,*) ':',(namestring(j),j=k1,k2),':'
          !namelen(i)= len_trim(namestring(k1:k1+9))
 !10     continue
        !write(x,*) 
                       
        !WRITE(x,*) I0,':',Opname,':',OpID,':',Section,':',(NameLen(i),':',(NameString(J),j=10*(i-1)+1,10*(i-1)+NameLen(i) ),i=1,Nval)

        OPEN (BINU,FILE=Binname(1:lei),ACCESS='SEQUENTIAL',FORM='UNFORMATTED', STATUS='UNKNOWN',IOSTAT=IOS,ERR=900)
  !      write(*,*) 'Write_Binout:first opened BINU on ',BINU
        !read(*,*) i

        WRITE (BINU) I0,Opname,OpID,Section,(NameLen(i),NameString(10*(i-1)+1:10*(i-1)+NameLen(i)),i=1,Nval)
      ELSE
        !append 
        OPEN (BINU,FILE=Binname,ACCESS='SEQUENTIAL',FORM='UNFORMATTED', STATUS='OLD',POSITION='APPEND',ERR=910)
  !      write(*,*) 'Write_Binout:append opened BINU on ',BINU

      END IF
      WRITE (BINU) I1,Opname,OpID,Section,UnitFlag,PrintLevel, (CurDate(I),I=1,5),(Values(I),I=1,Nval)

  !    write(*,*) 'Write_Binout:close ',BINU
      !read(*,*) i
 ...
 900  CONTINUE
        ...
        ERRFLG = 1
        i= len_trim(binname)
  !      WRITE (*,*) 'ERROR OPENING FILE',Binname(1:i), 'FOR WRITE',IOS-16384
        GO TO 999

 910  CONTINUE
        ERRFLG = 2
        i= len_trim(binname)
  !      WRITE (*,*) 'ERROR OPENING FILE',Binname(1:i), 'FOR APPEND',IOS-16384
        GO TO 999
