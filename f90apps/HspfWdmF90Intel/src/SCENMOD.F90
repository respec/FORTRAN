      MODULE SCENMOD
        ...
        PUBLIC M_SETDBG, M_SPIPH
        PUBLIC M_FILSTA
        PUBLIC UPDATESTATUS, UPDATESTATUSX
        PUBLIC M_GETSTRING,  SYNC_TIME
        PRIVATE
...
2000      FORMAT ("In M_SPIPH",2I5)
        ...
        !used for status messages in hspsta.for
...
2000      FORMAT(1X,A,I3,1X,A)
          ...
          WRITE(S,*) 'M_SIMSCN:about to call FILSET',MSG%FUN,UCI%FUN, &
                     WDM(1)%FUN,WDM(2)%FUN,WDM(3)%FUN,WDM(4)%FUN
          L = LEN_TRIM(S)
          CALL UPDATESTATUSX(I,L,S)
...
10        CONTINUE
 ...
 10       CONTINUE
        ...
        !used in hspsta.for by ihm-related code
 ...
 3                CONTINUE
                  IF (VARTYP(IV) .EQ. 2) THEN
                    WRITE (VSTR(IV),"(A6,1X,20I10))") &
                           VARNAM(IV),(IVAL(IV,IX),IX=1,MXSUB(IV))
                  ELSE IF (VARTYP(IV) .EQ. 3) THEN
                    WRITE (VSTR(IV),"(A6,1X,20(1PE12.4,1X))") &
                           VARNAM(IV),(RVAL(IV,IX),IX=1,MXSUB(IV))
                  END IF
 ...
 999      CONTINUE
        ...
        !used by ihm-related code in ext_update (hspsta.for) 
