...
c       modified            05 jun 2004  added gbthresh0 to subroutine call
...
c            gbthresh0  r*8  critical value for Grubbs-Beck test
c                            (-2 (q=0.01) or smaller => ignore gbcrit)
c
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
c       output variables:
...
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
      subroutine emafit(n,ql,qu,tl,tu,reg_skew,reg_mse,gbthresh0,
     ...
     2  cmoms(*),pq(*),yp(*),ci_low(*),ci_high(*),        ! output variables
     3  pqd(nq),eps(1),gbthresh0
...
c            gbthresh0  r*8  critical value for Grubbs-Beck test
c                            (0 or negative values = ignore gbcrit)
c
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
c       output variables:
...
c
       call gbtest(n,ql_in,qu_in,tl_in,tu_in,gbthresh0,ql,qu,tl,tu)
      
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
c   2.  begin by fitting the p3 dist'n to the at-site and regional data
c       
...
c
      call p3est_ema(n,ql,qu,1.d0,reg_skew,-99.d0,cmoms)
        as_mse   = mseg(n,cmoms(3))
      call p3est_ema(n,ql,qu,as_mse,reg_skew,reg_mse,cmoms)
        yp       = qP3(pq,cmoms)
...
c            common /tac001/gbcrit,gbthresh,nlow
c
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
      subroutine gbtest(n,ql_in,qu_in,tl_in,tu_in,gbthresh0,ql,qu,tl,tu)
     ...
     2  ql(*),qu(*),tl(*),tu(*),gbcrit,gbthresh,gbthresh0,
     3  x(10000),xm,xsum,xss,s,t,cmoms(3),gbtmin
      ...
      data gbtmin/-2.d0/
        ...
        nlow = 0
      if(gbthresh0 .gt. gbtmin) then
        gbthresh  = gbthresh0
      else                                            ! compute critical value
        ns   = 0
      do 10 i=1,n
...
10    continue
      if(ns .le. 5) then
        ...
        gbthresh = -99.d0
          ...
          call p3est_ema(ns,x,x,1.d0,0.d0,-99.d0,cmoms)
          xm      = cmoms(1)
          s       = sqrt(cmoms(2))
          t       = -0.9043+3.345*sqrt(log10(dble(ns)))
     1                         -0.4046*log10(dble(ns))
          gbcrit  = xm - s*t
          gbthresh= 1.d10
      endif
...
c
c     jfe modify set lower bound to zero
          ql(i) = 0.d0
        else
          ql(i) = ql_in(i)
          qu(i) = qu_in(i)
        endif
          tl(i) = tl_in(i)      !  max(tl_in(i),gbcrit)
          tu(i) = tu_in(i)
30    continue
      return
      end
...
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
C     double precision function rP3(m)
C     
C     implicit none
C     
C     double precision 
