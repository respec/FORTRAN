...
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
      subroutine emafit(n,ql,qu,tl,tu,reg_skew,reg_mse,
     ...
     3  pqd(nq),eps(1)
      ...
      data pqd/0.002,0.005,0.010,0.020,0.050,0.100,0.200,0.375,0.500,
     1         0.625,0.800,0.900,0.950,0.980,0.990,0.995,0.998/
     
      do 10 i=1,nq
          pq(i) = pqd(i)
        call emafitb(n,ql,qu,tl,tu,reg_skew,reg_mse,neps,eps,pq(i),
...
c
       call gbtest(n,ql_in,qu_in,tl_in,tu_in,ql,qu,tl,tu)
...
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
      subroutine gbtest(n,ql_in,qu_in,tl_in,tu_in,ql,qu,tl,tu)
     ...
     2  ql(*),qu(*),tl(*),tu(*),gbcrit,gbthresh,
     3  x(10000),xm,xsum,xss,s,t
...
20    continue
      ...
      do 30 i=1,n
        if(qu_in(i) .lt. gbcrit) then
          nlow  = nlow + 1
          qu(i) = gbthresh
c set lower bound to half the observed value
          ql(i) = qu_in(i)-log(2.d0)
        else
          ql(i) = ql_in(i)
          qu(i) = qu_in(i)
        endif
          tl(i) = max(tl_in(i),gbcrit)
          tu(i) = tu_in(i)
30    continue
      return
      end
...
30    continue
      if(bcf .eq. 1997) then
        ...
        n_bcf = n
      else if (bcf .eq. 0000) then
        n_bcf = 0
      endif
        c2          =  n_bcf/(n_bcf-1.)
        c3          =  ( n_bcf**2.)/( (n_bcf-1.)*(n_bcf-2.) )
