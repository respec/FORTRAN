...
C
  100 CONTINUE
        JSEQNO = JSEQNO + 1
        CALL PRTPHD( 1000 , JSEQNO)
C
        CALL INPUT (IA1,IA3,INFORM,MAXPKS,ISTART,STAID,PKS,IPKSEQ,
     $   XQUAL,IQUAL, NHIST,NSYS,HISTPD,    QHIOUT,QLWOUT,GAGEB,GENSKU,
     $         RMSEGS, IGSOPT, NSKIP1, IER )
C
        NPKS=NHIST+NSYS
        NSTAYR=NSTAYR+NPKS
        NSKIP=NSKIP+NSKIP1
...
C           print input data and plotting positions
            IF(IPRTOP .NE. 0 )  THEN      
              IF(IPPOS.EQ.0) THEN
C               short output
                CALL PRTIN2(0,MSG,NPKS,IPKSEQ,PKS,XQUAL)
              ELSE
C               longer output
...
C     + + + LOCAL VARIABLES + + +
      CHARACTER*1  BLANK
C     INTEGER   JDATE(3)
      INTEGER   I, DT(6), ERRCOD, OLEN, L70,L15,L48,L80,LOC,L10
C     INTEGER   JTIME
      ...
      CHARACTER*1 CHDTTM(21), HEADA9(80), CSTA(10)
C
C     + + + SAVES + + +
      SAVE  DISCLM          
...
C     + + + EXTERNALS + + +
C     EXTERNAL    DATTIM
      EXTERNAL    SYDATM, DATLST, LENSTR, CARVAR, CVARAR, CHRCHR
C
C      + + + DATA INITIALIZATIONS + + +
...
C    $  1A1,T21,66X,T21,    '  LOG-PEARSON CARDS              ' )
  199 FORMAT( '1' )
  200 FORMAT('  ')
  201 FORMAT( 21X, '      U. S. GEOLOGICAL SURVEY' )
C 202 FORMAT( 21X, 'OFFICE OF SURFACE WATER, RESTON, VA' )
  203 FORMAT( 21X, 'ANNUAL PEAK FLOW FREQUENCY ANALYSIS' )
  204 FORMAT( 21X, 'Following Bulletin 17-B Guidelines' )
  205 FORMAT( 21X, '          Program peakfq    ' )
  206 FORMAT( 21X, '     (Version 4.1, February, 2002)' )
  207 FORMAT( 20X, A40 )
  227 FORMAT(21A1)
...
C         assume before 2000
          DT(1) = DT(1) + 1900
        END IF
        CALL DATLST (DT,CHDTTM,OLEN,ERRCOD)
        WRITE(MSG1, 101) (CHDTTM(I),I=1,OLEN)
...
C       put date/time here for plots
        WRITE(HEAD7,227) (CHDTTM(I),I=1,OLEN)
...
C       change null to blank
 ...
 55     CONTINUE
        ...
        WRITE(MSG1, 100) (CHDTTM(I),I=1,OLEN)
...
C     + + + FORMATS + + +
    ...
    5 FORMAT(  
     ...
     $  /16X,'Standard error of generalized skew   = ',2X,A6,
     $  /16X,'Skew option                          = ',A,
...
C
 1021 FORMAT( //3X,
     ...
     $      / 73X, A, '** WEIBA =', F6.3, ' ***' )
 1022 FORMAT( 6X, 5HWATER, 9X, 6HRANKED, 7X,
     $       10HSYSTEMATIC, 6X,'BULL.17B'/ 
     $       7X,4HYEAR, 7X, 9HDISCHARGE, 8X, 6HRECORD,8X,8HESTIMATE/)
...
C
C     + + + EXTERNALS + + +
      EXTERNAL   SORTI, MOVEI, DECCHX, QEXTRA
C
C     + + + DATA INITIALIZATIONS + + +
      DATA     IPLIST / -777, 30*0 /
C
C     + + + FORMATS + + +
...
C     AND ORDINATES
      ...
      CALL PRTPHD ( 2001, -999 )
      WRITE(MSG,8)
      IF(IDEBUG.GT.0) WRITE(MSG,9) SYSAAV, SYSASD, SYSASK,
...
Ckmf  list arrays
Ckmf  write (99,3001) (iplist(i), wrcfc(i), sysrfc(i), climl(i),
Ckmf $                climu(i), i = 1, 31)
C3001 format ( '    iplist      wrcfc       sysrfc  ',
C    $         '    climl       climu   ',
C    $       / ( I8, 4X, 4f12.4 ) )
      LEN = 13
      SIGDIG = 4
...
Ckmf  call added to compute and print extra n-year floods
      TMP = 1.5
      CALL QEXTRA ( TMP, XTRPK )
      IF (XTRPK .GT. 0) THEN
        WRITE (MSG,2012) 1./TMP, XTRPK, TMP
      ELSE
        WRITE (MSG,2011) 1./TMP, TMP
      END IF
      TMP = 2.33
      CALL QEXTRA ( TMP, XTRPK )
      IF (XTRPK .GT. 0) THEN
...
C
      SUBROUTINE   INPUT
     #              (IA1,IA3,INFORM,MAXPKS,ISTART,STAID,PKSABG,
     $ IWYSN,XQUAL,IQUAL, NHIST,NSYS,HISTPD,  QHIOUT,QLWOUT,GAGEB,
     $     GENSKU, RMSEGS,ISKUOP,  NSKIP1,  IRC )
C
C     + + + PURPOSE + + +
C     RE-WRITTEN FOR PRIME VERSION 3.8-P,  WK, 7/88.
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER   IA1,IA3, INFORM, MAXPKS, NHIST, NSYS,
     &          ISKUOP, NSKIP1, IRC, ISTART
...
C
      ELSE IF ( INFORM .EQ. 1 )  THEN
        CALL  INPUT1( IA1,  IA3 ,
     $          MAXPKS, STAID, PKSABG, IWYSN, XQUAL,
     $       NHIST,NSYS,HISTPD,    QHIOUT,QLWOUT,GAGEB,GENSKU,
     $       RMSEGS,ISKUOP,  NSKIP1,  IRC )
C
      ELSE IF ( INFORM .EQ. 2 )  THEN
        CALL  INPUT2(IA1,MAXPKS,ISTART,STAID, PKSABG, IWYSN,  XQUAL,
     $       IQUAL, NHIST,NSYS,HISTPD,    QHIOUT,QLWOUT,GAGEB,GENSKU,
     $     RMSEGS,ISKUOP,  NSKIP1,  IRC )
C
...
C
     ...
     I                 (  MESSFL, MAXPKS,
     M                    ISTART,
     O                    STAID, PKSABG, IWYSN, XQUAL,
     O                    IQUAL, NHIST,NSYS,HISTPD,
     O                    QHIOUT,QLWOUT,GAGEB,GENSKU,
...
C
C     + + + EXTERNALS + + +
      EXTERNAL   PKFRD4, WCFGSM, IBITEX, LFTSTR, DOSTATION
...
C
  100 CONTINUE
        LOOPBK = 0
        CALL PKFRD4( INCRD,MSG1,  MAXPKS, ISTART,STAID, AUX, NOBS,
     $        PKSABG(21), IQUAL(21), IWYSN(21), IRC, XSYSPK,XHSTPK)
...
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER   IN, MSG, MXPKS, NPKS, IER, ISTART
...
C     + + + OUTPUT FORMATS + + +
 ...
 2001 FORMAT (    ' CARD types 4, 2, and * are ignored',
     $         /, 1X, A )
 2002 FORMAT (    ' Unrecognized CARD type.',
     $            '  Must be Y, Z, N, H, I, 2, 3, 4,  or *.',
     $         /, ' (2, 4, and * records are ignored.)',
     $         /, 1X, A )
 2003 FORMAT (    ' Error reading input lat. or long. on H card.',
     $         /, 1X, A )
 2004 FORMAT (   ' Error reading I-card',
     $         /, 1X, A )
 2005 FORMAT (   ' Error reading a 3 card.',
     $         /, 1X, A )
 2010 FORMAT (//, ' Insufficient data to process, only',
...
C     check if start/end on I card, if not, use systematic record
      ...
      IF (XHSTPK .GT. 1.0E29) XHSTPK = 0.0
C
      RETURN
      END
C
...
C     + + + DUMMY ARGUMENTS + + +
      CHARACTER*90  STAID
      REAL         WRCFC(31), SYSAV, SYSSD, SYSG, WRCAV, WRCSD,
     &             GWRC
...
C
C     + + + DATA INITIALIZATIONS + + +
      DATA    JPUN   /12,16,20,21,23,25,26,27,28/
...
C
      IF (IGKSPL .EQ. 1)  THEN
       write(*,*)'PLTFRQ: call GPINIT'
Cprh    init graphics here for now
        CALL GPINIT
Cprh    just use screen for now
C       set device type and code
       write(*,*)'PLTFRQ: call GPDEVC'
        CALL GPDEVC (1,1)
       write(*,*)'PLTFRQ: call FRQPLG'
        CALL FRQPLG
...
C
C     + + + PURPOSE + + +
C     For the specified recurrence interval, calculate the
C     corresponding peak.
C
C     + + + DUMMY ARGUMENTS + + +
      REAL      XYEAR, PEAK
C
C     + + + ARGUMENT DEFINTIONS + + +
C     XYEAR  - recurrence interval, in years
C     PEAK   - flood peak corresponding to XYEAR recurrence
C
C     + + + PARAMETERS + + +
      INCLUDE 'pmxint.inc'
C
...
C
C     + + + EXTERNALS + + +
      EXTERNAL   HARTIV, HARTK
C
C     + + + END SPECIFICATIONS + + +
C
      CALL HARTIV ( WRCSKW, WORK )
      Q    = WRCUAV + WRCUSD * HARTK ( (1.-1./XYEAR), WORK )
      QBAS = WRCUAV + WRCUSD * HARTK ( 1.-WRCPAB, WORK )
C
      IF (Q .LT. QBAS ) THEN
        PEAK = -999.
      ELSE
