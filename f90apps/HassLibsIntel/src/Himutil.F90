!     Last change:  AHA  30 Oct 102    5:14 pm
...
! Patrick Tara  3/11/02  Do not close file, File will be closed at the end of the simulation
      ...
      DIMENSION      xArray(nCol,nRow)
      CHARACTER(len=*) ::  INFNAME,OUTFNAME

      CHARACTER*16   TEXT
      INTEGER*4      IC,IR,IL,ILAY,nColFile,nRowFile,NLAY,NTRANS,NSPER,NTSTEP
      INTEGER*4      IERR,KSTP,KPER,ERRFLAG
      REAL*4         TOTIME,PERTIME
      logical        fileopened

      !DEC$ ATTRIBUTES DLLEXPORT ::  Read_MODFLOW_Unformatted
...
829   continue
      ...
      if(INFTYPE .eq. 0 .and. (.not. fileopened)) then  ! input file is binary
         !  WRITE(*,*)'Opening Input Binary File: ',INFNAME
           OPEN(UNIT=INFUNIT,FILE=INFNAME,RECORDTYPE='STREAM',SHARE='DENYNONE',STATUS='OLD',ERR=1929) !ACCESS='TRANSPARENT')
      else if(INFTYPE .eq. 1 .and. (.not. fileopened)) then   ! input file is unformatted
!              WRITE(*,*)'Opening Input Unformatted File: ',INFNAME
              OPEN(UNIT=INFUNIT,FILE=INFNAME,SHARE='DENYNONE',STATUS='OLD',ERR=1929,ACCESS='SEQUENTIAL',FORM='UNFORMATTED')  
      else
!             WRITE(*,*)' Input file is already opened'
      endif
...
1700  continue
      if(INFSOURCE .eq. 1) READ(INFUNIT,err=1930,end=1932) KSTP,KPER,PERTIME,TOTIME,TEXT,nColFile,nRowFile,ILAY ! READ IDENTIFYING RECORD
      if(INFSOURCE .eq. 2) READ(INFUNIT,err=1930,end=1932) NTRANS,KSTP,KPER,TOTIME,TEXT,nColFile,nRowFile,ILAY
   ...
   !      WRITE(*,2003)TOTIME
      ...
      if((INFSOURCE .eq. 1 .or. INFSOURCE .eq. 2) .and. INFTYPE .eq. 0) then ! binary input file
         DO IR=1,NROW
            DO IC=1,NCOL
               READ(INFUNIT,err=1930,end=1932)xArray(IC,IR)
              ! write(*,*) IR,IC,layNo,xArray(IC,IR)
            END DO
         END DO
      else if((INFSOURCE .eq. 1 .or. INFSOURCE .eq. 2) .and. INFTYPE .eq. 1) then  ! unformatted input file
         READ(INFUNIT,err=1930,end=1932)((xArray(IC,IR),IC=1,NCOL),IR=1,NROW)
      endif
...
1931  continue
  ...
  !    write(*,*)' Error: NOT ENOUGH MEMORY'
        goto 1933
1932  continue
      ERRFLAG = 3    ! END OF FILE
  !    write(*,*)' Error/Warning: End of File Reached'
      goto 1933
1933  continue
      ...
      END
!_________________________________________________________________________________________
function Read_MODFLOW_3D_Unformatted(xArray,nCol,nRow,nLay,infSource,infType,infUnit,infName,outfType,outfUnit,outfName)
! ************************************************************
! Reads Unformatted Files (Head, Drawdown, or Cell-by-Cell Flow) Written by MODFLOW
! and stores one in xArray.
!       xArray                  Array to be filled by this function
...
! Patrick Tara  3/11/02  Do not close file, File will be closed at the end of the simulation
      ...
      IMPLICIT NONE
      INTEGER*4      Read_MODFLOW_3D_Unformatted
      REAL*4         xArray
      INTEGER*4      nCol,nRow,nLay,infSource,infType,infUnit,outfType,outfUnit
      DIMENSION      xArray(nCol,nRow)
      CHARACTER(len=*) ::  INFNAME,OUTFNAME

      CHARACTER*16   TEXT
      INTEGER*4      IC,IR,IL,ILAY,layNo,nColFile,nRowFile,NTRANS,NSPER,NTSTEP
      INTEGER*4      IERR,KSTP,KPER,ERRFLAG,IMETH
      REAL*4         DELT,PERTIM,TOTIM
      logical        fileopened

      !DEC$ ATTRIBUTES DLLEXPORT ::  Read_MODFLOW_3D_Unformatted

      Read_MODFLOW_3D_Unformatted = 0
...
1700  continue
      if(INFSOURCE .eq. 3) READ(INFUNIT,err=1930,end=1932) KSTP,KPER,TEXT,nColFile,nRowFile,layNo

      ! Alaa - change this later layNo = (-1)*layNo
      ...
      if(INFSOURCE .eq. 3) READ(INFUNIT,err=1930,end=1932) IMETH,DELT,PERTIM,TOTIM


      if(INFSOURCE .eq. 3 .and. INFTYPE .eq. 0) then ! binary 3D file
            DO IR=1,NROW
               DO IC=1,NCOL
                   READ(INFUNIT,err=1930,end=1932) xArray(IC,IR)
               END DO
...
! cancel the following two statements to read one array only as specified by COMPACT BUDGET FILES      
!      IF((infSource .eq. 1 .OR. infSource .eq. 2) .AND. iLay .eq. layNo) GOTO 1933
!      goto 1700
      goto 1933
...
1933  continue
      ...
      Read_MODFLOW_3D_Unformatted = ERRFLAG
      return
      END
!_________________________________________________________________________________________
...
! Patrick Tara  3/11/02  Do not close file, File will be closed at the end of the simulation
      ...
      !DEC$ ATTRIBUTES DLLEXPORT ::  Read_MODFLOW_List
...
829   continue
   ...
   !        WRITE(*,*)'Opening Output Binary File: ',OUTFNAME
           OPEN(UNIT=OUTFUNIT,FILE=OUTFNAME,STATUS='REPLACE',ACTION='WRITE') !,ACCESS='TRANSPARENT'
...
! last modified: 11-29-01
      ...
      !DEC$ ATTRIBUTES DLLEXPORT ::  Read_Integer_Array
...
! Check if input file is open
      ...
      IF(.NOT. OPD) THEN
         ...
         IF(infUnit .lt. 0) OPEN(UNIT=IABS(infUnit),FILE=infName,STATUS='OLD',ERR=469,ACTION='READ') !,ACCESS='TRANSPARENT'
      ENDIF
...
!--CHECK IF THE EXTERNAL FILE HAS BEEN OPENED.  IF NOT, OPEN
  250 INQUIRE(UNIT=IABS(IREAD), OPENED = OPD)
      IF(.NOT. OPD) THEN
         ...
         ENDIF
         IF(IREAD .lt. 0) then
             OPEN(UNIT=IABS(IREAD),FILE=infName,STATUS='OLD',ERR=469,ACTION='READ') !,ACCESS='TRANSPARENT'
             READ(IABS(IREAD))
             READ(IABS(IREAD)) IA
         ENDIF
      ENDIF
...
! last modified: 11-29-01
      ...
      !DEC$ ATTRIBUTES DLLEXPORT ::  Read_Real_Array
...
! last modified: 11-29-01
      ...
      !DEC$ ATTRIBUTES DLLEXPORT ::  Write_Integer_Array
...
! Check if output file is open
      ...
      IF(.NOT. OPD) THEN
         IF(outfUnit .gt. 0 .and. outfType .eq. 0) then
             ...
             OPEN(UNIT=outfUnit,FILE=outfName,POSITION='APPEND',ERR=469,ACCESS='SEQUENTIAL',FORM='FORMATTED',ACTION='WRITE')
             ...
             OPEN(UNIT=outfUnit,FILE=outfName,POSITION='REWIND',ERR=469,ACCESS='SEQUENTIAL',FORM='FORMATTED',ACTION='WRITE')
             ...
             OPEN(UNIT=-outfUnit,FILE=outfName,POSITION='APPEND',ERR=469,RECORDTYPE='STREAM',ACTION='WRITE')
             ...
             OPEN(UNIT=-outfUnit,FILE=outfName,POSITION='REWIND',ERR=469,RECORDTYPE='STREAM',ACTION='WRITE')
         ENDIF
      ENDIF
...
! last modified: 11-29-01
      ...
      !DEC$ ATTRIBUTES DLLEXPORT ::  Write_Real_Array
...
! Check if output file is open
      ...
      IF(.NOT. OPD) THEN
         IF(outfUnit .gt. 0 .and. outfType .eq. 0) then
             ...
             !OPEN(UNIT=-outfUnit,FILE=outfName,STATUS='REPLACE',ERR=469,ACCESS='TRANSPARENT',ACTION='WRITE')
             OPEN(UNIT=-outfUnit,FILE=outfName,STATUS='REPLACE',ERR=469,RECORDTYPE='STREAM',ACTION='WRITE')
        ENDIF
      ENDIF
...
function Write_Binout(Binname,Nval,Values,NameLen,NameString,Opname,OpID,Section,UnitFlag,PrintLevel,CurDate,FirstLoop,OpenFlag)
! ****************************************************************
...
! FirstLoop     1 if first time through, so must write headers, else 0
! OpenFlag      1 if first segment in loop, so open file; 2 if last so close, else 0
! 
! ****************************************************************
...
! last modified: 27-Sep-2002
      ...
      INTEGER       Nval,NameLen(Nval),OpID,UnitFlag,PrintLevel,CurDate(6),FirstLoop,OpenFlag
      REAL          Values(Nval)
      CHARACTER(len=*) ::  Binname, NameString, Opname, Section 

      INTEGER       I,I0,I1,BINU,IOS,ERRFLG
      CHARACTER*8   lOpname, lSection

      DATA I0,I1,BINU/0,1,151/

      !DEC$ ATTRIBUTES DLLEXPORT ::  Write_Binout

      ERRFLG = 0

      WRITE (lOpname,2000) (Opname(I:I),I=1,6)
      WRITE (lSection,2000) (Section(I:I),I=1,6)
      !write (*,*) 'WriteBinout:',Opname,':',lOpname,':',Section,':',lSection,':'
 2000 FORMAT (6A1,'  ')

      IF (OpenFlag .eq. 1) THEN
        IF (FirstLoop .eq. 1) THEN
          !open new file to write
          !OPEN (BINU,FILE=Binname,ACCESS='SEQUENTIAL',FORM='UNFORMATTED',STATUS='UNKNOWN',ACTION='DENYNONE',IOSTAT=IOS,ERR=900)
          OPEN (BINU,FILE=Binname,ACCESS='SEQUENTIAL',FORM='UNFORMATTED',STATUS='UNKNOWN',SHARE='DENYNONE',IOSTAT=IOS,ERR=900)
        ELSE
          !open the file this interval for append
          I= 0
 10       CONTINUE
            OPEN (BINU,FILE=Binname,ACCESS='SEQUENTIAL',FORM='UNFORMATTED', STATUS='OLD',POSITION='APPEND',IOSTAT=IOS,ERR=20)
 20         CONTINUE
            I =I+ 1
          IF (IOS .NE. 0 .AND. I .LT. 1000) GO TO 10
      ...
      IF (FirstLoop .eq. 1) THEN
        ...
        WRITE (BINU) I0,lOpname,OpID,lSection,(NameLen(i),NameString(10*(i-1)+1:10*(i-1)+NameLen(i)),i=1,Nval)
      END IF

      !always write data
      WRITE (BINU) I1,lOpname,OpID,lSection,UnitFlag,PrintLevel, (CurDate(I),I=1,5),(Values(I),I=1,Nval)

      IF (OpenFlag .EQ. 2) THEN
        !last time through this interval
        CLOSE (BINU)
      END IF
