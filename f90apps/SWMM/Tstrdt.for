      SUBROUTINE TSTRDT
C     TRANSPORT BLOCK
C     CALLED BY INTRAN NEAR LINE 877
C=======================================================================
...
C     FIX COEFFICIENTS IN TWO POWER EQUATIONS, WCH, 10/20/95.
C=======================================================================
      INCLUDE 'TAPES.INC'
      INCLUDE 'INTER.INC'
      INCLUDE 'HUGO.INC'
...
C#### WCH, 11/3/92
      IF(I.EQ.1) THEN
C#### WCH, 10/20/95. SHOULD BE D01, NOT D0(I)!!! Aarrgghh!
           IF(TSDEP(IS,MM)-D01.GT.0.0001) TSQOU(IS,MM)=TSQOU(IS,MM)+
     1         A11*(TSDEP(IS,MM)-D01)**A21
                 ENDIF
           IF(I.EQ.2) THEN
C#### WCH, 10/20/95.  SHOULD BE D0(IS), NOT D0(I)!!! Aaarggh!
                IF(TSDEP(IS,MM)-D0(IS).GT.0.0001) TSQOU(IS,MM)=
     1               TSQOU(IS,MM)+A1(IS)*(TSDEP(IS,MM)-D0(IS))**A2(IS)
                ENDIF
  420 CONTINUE
      GO TO 600
C=======================================================================
...
C=======================================================================
  220 FORMAT(/,' ===> ERROR !!! IN DATA GROUP G2 : THE FIRST ENTRIES FOR
     1DEPTH AND VOLUME FOR STORAGE ELEMENT ',I10,/,32X,'ARE NON-ZERO. SI
  ...
  510 FORMAT(/,' ===> ERROR !!! IN DATA GROUP G4 : IN STORAGE ELEMENT ',
     1I10,' THE DEPTH AT WHICH THE SECOND PUMPING RATE BEGINS IS LESS',/
     2,32X,'THAN THE DEPTH AT WHICH THE FIRST PUMPING RATE BEGINS. SIMUL
     3ATION TERMINATED.')
  ...
  520 FORMAT(/,' ===> ERROR !!! IN DATA GROUP G4 : IN STORAGE ELEMENT ',
     1I10,' THE DEPTH AT WHICH ALL PUMPING STOPS IS GREATER THAN',/,32X,
 ...
 1030 FORMAT(10X,23('*'),/,10X,'STORAGE ELEMENT # ',I10,/,10X,23('*'),/)
