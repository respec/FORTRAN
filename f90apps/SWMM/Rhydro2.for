      SUBROUTINE RHYDRO2
C	RUNOFF BLOCK
C	CALLED BY SUBROUTINE RUNOFF NEAR LINE 229
C=======================================================================
C     SECOND GENERAL INPUT SUBROUTINE OF THE RUNOFF BLOCK
C     RHYDRO2 LAST UPDATED MAY, 1989
C     WCH, 10/19/95.  ADD CHECK FOR POSSIBLE WRONG INCLUSION OF QUALTY.
C=======================================================================
      INCLUDE 'TAPES.INC'
      INCLUDE 'INTER.INC'
      INCLUDE 'STIMER.INC'
      ...
      INCLUDE 'NEW89.INC'
C=======================================================================
C#### WCH, 10/19/95.  Add check for possible inclusion of quality data.
C=======================================================================
      READ(N5,*,ERR=888) CC
      BACKSPACE N5
      IF(CC.EQ.'JJ'.OR.CC.EQ.'J1') THEN
           WRITE(N6,1940)
           STOP ' Run stopped while trying to input M1 line.'
           ENDIF
C=======================================================================
...
C=======================================================================
      ...
      IF(NPRNT.GT.0) THEN
               ...
               DO I = 1,NDET
               JYEAR = JSTART(I)/10000
               IF ((JSTART(I).GT.0).AND.(JYEAR.LT.100)) THEN
               JSTART(I) = JSTART(I) - JYEAR*10000
               JYEAR = JYEAR + 1900
               JSTART(I) = JSTART(I) + JYEAR*10000
               ENDIF
               JYEAR = JSTOP(I)/10000
               IF ((JSTOP(I).GT.0).AND.(JYEAR.LT.100)) THEN
               JSTOP(I) = JSTOP(I) - JYEAR*10000
               JYEAR = JYEAR + 1900
               JSTOP(I) = JSTOP(I) + JYEAR*10000
               ENDIF
               ENDDO
...
C=======================================================================
 ...
 2040 NGUT(J)   = 0
  ...
  888 CALL IERROR
C=======================================================================
C#### WCH, 10/19/95.  NEW FORMAT STATEMENT.
 1940 FORMAT(' ===> ERROR!! PROGRAM EXPECTS M1 LINE BUT FINDS QUALITY DA
     1TA.',/,' REMOVE QUALITY DATA OR SET KWALTY = 1 ON B1 LINE.')
 1955 FORMAT(/,
     +'     ***********************',/,
     +'     *    DATA GROUP M1    *',/,
     +'     ***********************',//,
