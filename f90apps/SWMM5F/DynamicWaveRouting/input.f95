...
integer function input_countObjects()
!
!  Input:   none
...
!    int   sect = -1, newsect;          ! input data sections          
    ...
    integer ::   errsum                  ! number of errors found                   
    integer ::   i 
!    long  lineCount = 0;
...
!    if ( errsum > 0 ) ErrorCode = ERR_INPUT;
    ...
    input_countObjects = ErrorCode
    return
end function input_countObjects
!
!//=============================================================================
!
!int input_readData()
...
integer function addObject(objType, id)
!
...
!
!  this is supposed to be called by 'input_countObject()'
!  but we will call it from driver program in this project
    ...
    {
      case (s_RAINGAGE)
        if ( .not. project_addObject(GAGE, id, Nobjects(GAGE)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(GAGE) = Nobjects(GAGE) + 1
        !break

      case (s_SUBCATCH)
        if ( .not. project_addObject(SUBCATCH, id, Nobjects(SUBCATCH)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(SUBCATCH) = Nobjects(SUBCATCH) + 1
        !break

      case (s_AQUIFER)
        if ( .not. project_addObject(AQUIFER, id, Nobjects(AQUIFER)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(AQUIFER) = Nobjects(AQUIFER) + 1
        !break

      case (s_UNITHYD)
        ! --- the same Unit Hydrograph can span several lines
        if ( project_findObject(UNITHYD, id) < 0 ) then
            if ( .not. project_addObject(UNITHYD, id, Nobjects(UNITHYD)) ) &
               &errcode = error_setInpError(ERR_DUP_NAME, id)
            Nobjects(UNITHYD) = Nobjects(UNITHYD) + 1
        end if
        !break

      case (s_SNOWMELT)
        ! --- the same Snowmelt object can appear on several lines
        if ( project_findObject(SNOWMELT, id) < 0 ) then
            if ( .not. project_addObject(SNOWMELT, id, Nobjects(SNOWMELT)) ) &
               &errcode = error_setInpError(ERR_DUP_NAME, id)
            Nobjects(SNOWMELT) = Nobjects(SNOWMELT) + 1
        end if
        !break

      case (s_JUNCTION)
        if ( .not. project_addObject(NODE, id, Nobjects(NODE)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(NODE) = Nobjects(NODE) + 1
        Nnodes(JUNCTION) = Nnodes(JUNCTION) + 1
        !break
      ...
      case (s_OUTFALL)
        if ( .not. project_addObject(NODE, id, Nobjects(NODE)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(NODE) = Nobjects(NODE) + 1
        Nnodes(OUTFALL) = Nnodes(OUTFALL) + 1
        !break
      ...
      case (s_STORAGE)
        if ( .not. project_addObject(NODE, id, Nobjects(NODE)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(NODE) = Nobjects(NODE) + 1
        Nnodes(STORAGE) = Nnodes(STORAGE) + 1
        !break
      ...
      case (s_DIVIDER)
        if ( .not. project_addObject(NODE, id, Nobjects(NODE)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(NODE) = Nobjects(NODE) + 1
        Nnodes(DIVIDER) = Nnodes(DIVIDER) + 1
        !break

      case (s_CONDUIT)
        if ( .not. project_addObject(LINK, id, Nobjects(LINK)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(LINK) = Nobjects(LINK) + 1
        Nlinks(CONDUIT) = Nlinks(CONDUIT) + 1
        !break
      ...
      case (s_ORIFICE)
        if ( .not. project_addObject(LINK, id, Nobjects(LINK)) ) & 
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(LINK) = Nobjects(LINK)+1
        Nlinks(ORIFICE) = Nlinks(ORIFICE)+1
        !break
      ...
      case (s_OUTLET)
        if ( .not. project_addObject(LINK, id, Nobjects(LINK)) ) &
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(LINK) = Nobjects(LINK)+1
        Nlinks(OUTLET) = Nlinks(OUTLET)+1
        !break

      case (s_POLLUTANT)
        if ( .not. project_addObject(POLLUT, id, Nobjects(POLLUT)) ) & 
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(POLLUT) = Nobjects(POLLUT)+1
        !break

      case (s_LANDUSE)
        if ( .not. project_addObject(LANDUSE, id, Nobjects(LANDUSE)) ) & 
           &errcode = error_setInpError(ERR_DUP_NAME, id)
        Nobjects(LANDUSE) = Nobjects(LANDUSE)+1
        !break

      case (s_PATTERN)
        ! --- a time pattern can span several lines
        if ( project_findObject(TIMEPATTERN, id) < 0 ) then
            if ( .not. project_addObject(TIMEPATTERN, id, Nobjects(TIMEPATTERN)) ) &
               &errcode = error_setInpError(ERR_DUP_NAME, id)
            Nobjects(TIMEPATTERN) = Nobjects(TIMEPATTERN)+1
        end if
        !break
...
!        {
!            if ( .not. project_addObject(CURVE, id, Nobjects(CURVE)) ) &
!               &errcode = error_setInpError(ERR_DUP_NAME, id)
!            Nobjects(CURVE) = Nobjects(CURVE)+1
!
...
!                Nobjects(SHAPE) = Nobjects(SHAPE)+1
!        }
!        !break

      case (s_TIMESERIES)
        ! --- a Time Series can span several lines
        if ( project_findObject(TSERIES, id) < 0 ) then
            if ( .not. project_addObject(TSERIES, id, Nobjects(TSERIES)) ) &
               &errcode = error_setInpError(ERR_DUP_NAME, id)
            Nobjects(TSERIES) = Nobjects(TSERIES)+1
        end if
        !break

      case (s_CONTROL)
        if ( match(id, w_RULE) ) Nobjects(CONTROL) = Nobjects(CONTROL)+1
        !break
...
!            if ( id ) then
!                if ( .not. project_addObject(TRANSECT, id, Nobjects(TRANSECT)) ) &
!                   &errcode = error_setInpError(ERR_DUP_NAME, id)
!                Nobjects(TRANSECT) = Nobjects(TRANSECT)+1
!            end if
!        end if
!        !break
...
!!  Following segment added for SWMM5-LID edition  !!                      !(5.0.019 - LR)
      case (s_LID_CONTROL)
        ...
        if ( project_findObject(LID, id) < 0 ) then
            if ( .not. project_addObject(LID, id, Nobjects(LID)) ) &
               &errcode = error_setInpError(ERR_DUP_NAME, id)
            Nobjects(LID) = Nobjects(LID)+1
        end if
        !break
    end select
