...
!
real(kind=dp) function getMixedQual(c, v1, wIn, qIn, tStep)
!
!  Input:   c = pollutant concentration at start of time step (mass/ft3)
...
!  
    ...
    integer ::    p        ! pollutant index
    real(kind=dp) :: qIn   ! inflow rate (cfs)
    real(kind=dp) :: wIn   ! pollutant mass inflow rate (mass)
...
!
    ...
    implicit none
    integer, intent(in) :: j
    real(kind=dp), intent(in) :: v, q, tStep
    integer ::    k
    real(kind=dp) :: hrt
...
!!!  New function added to release 5.0.017. !!                              !(5.0.017 - LR)
!
real(kind=dp) function getReactedQual(p, c, v1, tStep)
!
...
!
    ...
    implicit none
    integer, intent(in) :: p
    real(kind=dp), intent(in) :: c, v1, tStep
