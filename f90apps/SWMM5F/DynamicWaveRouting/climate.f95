...
integer, parameter :: UNKNOWN_FORMAT = 1
integer, parameter :: USER_PREPARED = 2     ! SWMM 5's own user format
integer, parameter :: TD3200 = 3            ! NCDC TD3200 format
integer, parameter :: DLY0204 = 4          ! Canadian DLY02 or DLY04 format
!}
integer, parameter :: MAXCLIMATEVARS  = 4
integer, parameter :: MAXDAYSPERMONTH = 32

! These variables are used when processing climate files.
!enum   ClimateVarType {
integer, parameter :: E_TMIN = 1 
integer, parameter :: E_TMAX = 2 
integer, parameter :: E_EVAP = 3 
integer, parameter :: E_WIND = 4
!}
character(4), dimension(4), parameter :: ClimateVarWords = (/'TMIN', 'TMAX', 'EVAP', 'WDMV' /)
...
!
    ...
    if ( Wind%datatype == FILE_WIND .or. Evap%datatype == FILE_EVAP .or. &                   !(5.0.016 - LR)
        &Evap%datatype == TEMPERATURE_EVAP ) then                                       !(5.0.016 - LR)
        if ( Fclimate%mode == NO_FILE ) then
            call report_writeErrorMsg(ERR_NO_CLIMATE_FILE, '')
        end if
    end if

    ! --- snow melt parameters tipm & rnm must be fractions
    if ( Snow%tipm < 0.0 .or. &                                                    !(5.0.012 - LR)
        &Snow%tipm > 1.0 .or. &                                                !(5.0.012 - LR)
        &Snow%rnm  < 0.0 .or. &                                                 !(5.0.012 - LR)
        &Snow%rnm  > 1.0 ) call report_writeErrorMsg(ERR_SNOWMELT_PARAMS, '')      !(5.0.012 - LR)

    ! --- latitude should be between -90 & 90 degrees                         !(5.0.013 - LR)
    a = Temp%anglat
    if ( a <= -89.99 .or. &                                                        !(5.0.013 - LR)
        &a >= 89.99  ) then 
        call report_writeErrorMsg(ERR_SNOWMELT_PARAMS, '')          !(5.0.013 - LR)
    else 
        Temp%tanAnglat = tan(a * PI / 180.0)                                 !(5.0.012 - LR)
    end if
    ...
    if ( z <= 0.0 ) then
        ...
        pa = 29.9 - 1.02*z + 0.0032 * (z ** 2.4) ! atmos. pressure
    end if
    Temp%gamma = 0.000359 * pa
end subroutine climate_validate
!
!!=============================================================================
!
...
!    FileValue(E_TMIN) = Temp.ta
!    FileValue(E_TMAX) = Temp.ta
!    FileValue(E_EVAP) = 0.0
!    FileValue(E_WIND) = 0.0
!
...
!        {
!            Tmin = FileValue(E_TMIN)
!            Tmax = FileValue(E_TMAX)
...
!                FileValue(E_EVAP) = getTempEvap(day)                            !(5.0.016 - LR)
!        }
!
...
!
!      case FILE_EVAP:
!        Evap.rate = FileValue(E_EVAP) / UCF(EVAPRATE)
!        datetime_decodeDate(theDate, &yr, &mon, &day)
...
!
!    ! --- process EVAP
!    if ( strlen(s2) > 0 && *s2 != '*' ) FileData(E_EVAP)(d) = atof(s2)
!
!    ! --- process WIND
!    if ( strlen(s3) > 0 && *s3 != '*' ) FileData(E_WIND)(d) = atof(s3)
!}
!
!!=============================================================================
!
...
!    p = atoi(param)
!    if ( p == 1 ) p = E_TMAX
!    else if ( p == 2 ) p = E_TMIN
!    else if ( p == 151 ) p = E_EVAP                                             !(5.0.014 - LR)
!    else return
!
