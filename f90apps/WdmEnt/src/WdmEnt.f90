 ...
 99     CONTINUE
    ...
    FUNCTION F90_WDMOPN(UNIT,NAME)
        ...
        IF (OPEN) THEN
          F90_WDMOPN= 1
        ELSE
          OPEN(UNIT=UNIT,FILE=NAME,ACCESS='DIRECT',RECL=512,STATUS='OLD',ERR=99)
          F90_WDMOPN= 0
        END IF
        RETURN
 99     CONTINUE
 ...
 99     CONTINUE
    ...
    SUBROUTINE F90_WDBSGCX (WDMSFL,DSN,SAIND,SALEN,CVAL)
        !DEC$ ATTRIBUTES DLLEXPORT :: F90_WDBSGCX
        !DEC$ ATTRIBUTES REFERENCE :: CVAL
        INTEGER           :: WDMSFL,DSN,RETCOD,I,SAIND,SALEN
        CHARACTER(LEN=256):: CVAL

        CVAL = ""
        CALL WDBSGC(WDMSFL,DSN,SAIND,SALEN, &
                    CVAL,RETCOD)
    END SUBROUTINE F90_WDBSGCX
