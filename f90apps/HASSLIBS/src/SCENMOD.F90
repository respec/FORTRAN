...
2000      FORMAT(1X,A,I3,1X,A)
          ...
          CALL INVALOP (LFLAG)
          CALL UNDFL (LFLAG)
C          CALL OVEFL (LFLAG)    dangerous to avoid these
C          CALL DVCHK (LFLAG)
...
10        CONTINUE
        ...
        SUBROUTINE M_GLOBLK (SDATE,EDATE,OUTLEV,
     ...
     O                   CBUFF,RECTYP,RETCOD)
        ...
        !newaqt:scnutl
        SUBROUTINE M_XTABLE (OMCODE,TABNO,UUNITS,INIT,ADDFG,OCCUR,
     O                       RETKEY,IBUFF,RETCOD)
          ...
          IF (MSG%FUN > 0) THEN
            CALL XTABLE (MSG%FUN,OMCODE,TABNO,UUNITS,INIT,ADDFG,OCCUR,
     O                   RETKEY,CBUFF,RETCOD)
          ELSE
        ...
        !newaqt:scnutl
        SUBROUTINE M_XTABLEEX (OMCODE,TABNO,UUNITS,INIT,ADDFG,OCCUR,
     M                         RETKEY,
     O                         IBUFF,RECTYP,RETCOD)
          ...
          IF (MSG%FUN > 0) THEN
            CALL XTABLEEX (MSG%FUN,OMCODE,TABNO,UUNITS,INIT,ADDFG,OCCUR,
     M                     RETKEY,
     O                     CBUFF,RECTYP,RETCOD)
          ELSE
            WRITE(*,*) 'SCENMOD:M_XTABLE:no message file open'
        ...
        !newaqt:scnutl
        SUBROUTINE M_XTINFO (OMCODE,TNUM,UUNITS,CHKEST,
     O              LNFLDS,LSCOL,LFLEN,ILFTYP,LAPOS,LIMIN,LIMAX,
     O              LIDEF,LRMIN,LRMAX,LRDEF,LNMHDR,IHDRBUF,
     O              IFDNAM,ISECT,IREPT,RETCOD)

        INTEGER,INTENT(IN)  :: OMCODE,TNUM,UUNITS,CHKEST
        INTEGER,INTENT(OUT) :: LNFLDS,LSCOL(30),LFLEN(30),
     1                         LAPOS(30),LIMIN(30),LIMAX(30),
     2                         LIDEF(30),LNMHDR,RETCOD,ISECT,IREPT
        ...
        CALL XCLUGP (MSG%FUN,OMCODE,TNUM,UUNITS,CHKEST,
 ...
 10       CONTINUE
        ...
        SUBROUTINE M_SET_DRIVER (INTEG_FLAG,INTEG_TIMESTEP,
     1                           INTEG_FILENAME)
